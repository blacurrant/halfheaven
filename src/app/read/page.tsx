  ...
  return (
    <main style={{ maxWidth: 780, margin: "0 auto", padding: "44px 20px 90px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 14, flexWrap: "wrap" }}>
        <button className="h-[38px] px-4 rounded-full font-semibold text-sm inline-flex items-center gap-1.5 border-[1.5px] bg-[var(--accent)] border-[var(--accent)] text-[var(--on-accent)] hover:bg-[var(--accent-press)] whitespace-nowrap" disabled={!file || job?.status === "running"} onClick={start}>
      ...
      {fp && (
        <section style={{ marginTop: 30 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 14 }}>
            {groups.map((group) => (
              <div key={group.key} className="flex flex-col rounded-[18px] bg-[var(--card)] border border-[var(--line)]" style={{ padding: "13px 15px 14px" }}>
          ...
          {missing.length > 0 && (
            <div className="flex flex-col rounded-[18px] bg-[var(--card)] border border-[var(--line)]" style={{ marginTop: 10, padding: "13px 15px 14px" }}>
