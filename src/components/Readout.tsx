...
export default function Readout({ fp }: { fp: Fingerprint }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
      {groups.map((g) => (
        <div key={g.key} className="flex flex-col rounded-[18px] bg-[var(--card)] border border-[var(--line)]" style={{ padding: "12px 14px 13px" }}>
      ...
      {missing.length > 0 && (
        <div className="flex flex-col rounded-[18px] bg-[var(--card)] border border-[var(--line)]" style={{ padding: "12px 14px 13px" }}>
